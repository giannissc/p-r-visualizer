...
impl MazeGenerationAlgorithm for RecursiveBacktrace {
    ...
    fn next_step(
        ...
        if self.algorithm_state == MazeAlgorithmState::Initialization {
            ...
            grid.clear_all();
            grid.add_node_perimeter(
                GridNodePosition { row: 0, col: 0 },
                GRID_ROWS,
                GRID_COLUMNS,
                GridNodeType::Wall,
                1,
    ...
    fn get_next_node(&mut self, grid: &mut Grid) -> Option<MazeNodes> {
        ...
        for (index, first__order_neighbour) in grid
            .available_neighbours_rectilinear(self.current_maze_node.position)
            .iter()
            .enumerate()
        {
            match first__order_neighbour {
                ...
                Some(first_order_neighbour_position) => {
                    let first_neighbour_node = MazeNodes::new(
                        *first_order_neighbour_position,
                        Some(self.current_maze_node.position),
                    );
                    // Get 1st order neighbours. Discard those already in closed list
                    if !self.closed_list.contains(&first_neighbour_node) {
                        ...
                        if index == 0 || index == 1 {
                            ...
                            // above and below
                            grid.add_node(
                                &first_order_neighbour_position.left(),
                                GridNodeType::Wall,
                                1,
                            );
                            grid.add_node(
                                &first_order_neighbour_position.right(),
                                GridNodeType::Wall,
                                1,
                            );
                            ...
                            if index == 0 {
                                second_order_option =
                                    grid.available_above(*first_order_neighbour_position);
                            } else {
                                second_order_option =
                                    grid.available_below(*first_order_neighbour_position);
                            }
                        } else {
                            // For each successful neighbour shade their sides (add wall)
                            // left and right
                            grid.add_node(
                                &first_order_neighbour_position.above(),
                                GridNodeType::Wall,
                                1,
                            );
                            grid.add_node(
                                &first_order_neighbour_position.below(),
                                GridNodeType::Wall,
                                1,
                            );
                            ...
                            if index == 2 {
                                second_order_option =
                                    grid.available_left(*first_order_neighbour_position);
                            } else {
                                second_order_option =
                                    grid.available_right(*first_order_neighbour_position);
                            }
                        }
                        ...
                        match second_order_option {
                            ...
                            Some(second_order_neighbour_position) => {
                                ...
                                let second_neighbour_node = MazeNodes::new(
                                    second_order_neighbour_position,
                                    Some(self.current_maze_node.position),
                                );
